...
SpMat Hessian_mat(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	for(i = 0; i < n; ++i) {
		...
		//temp_vec = W.row(i);
		// k and k1 denotes the index w.r.t. which deriv is taken -- W_ik W_ik1 
		for(k = 0; k < 3; ++k) {
			//for(k1 = 0; k1 < 3; ++k1) {
			for(k1 = k; k1 < 3; ++k1){
				...
				// The final answer involves a sum over j for each pixel and k, k1
				for(j = 0; j < m ; ++j) {
...
//					if(i == 0){
//						Debug0("i: " << i << ", j: " << j << ", k: " << k << ", k1: " << k1);
//						Debug0("components: " << (tmp3 * simple_dee_2_v_ij_dee_W_ik_dee_W_ik1(W.row(i), TE, TR, j, k, k1) + 
//								tmp4 * simple_dee_v_ij_dee_W_ik(W.row(i), TE, TR, j, k) * 
//									simple_dee_v_ij_dee_W_ik(W.row(i), TE, TR, j, k1)) << "\n");
//					}
				}
				...
				*/
				...
				// Numerical Fix for te ediagonal part: 
				if(temp > 0.0){
					temp = -1e-6;
				}
