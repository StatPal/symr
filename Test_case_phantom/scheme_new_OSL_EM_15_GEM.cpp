...
All images can be done at once for the parametric bootstrap case - change that...
...
// Change
	...
	if(penalized){
		...
		crit_MRF.iterations = 500;														// number of allowed iterations
...
Matrix_eig Var_est_test_mat(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	assert(A.rows() == 3*n);
	save_sparse(A, "Hessian_Matrix_15.csv", 1);
...
int main(int argc, char * argv[]) {
	...
	file_performance.open ("result/Performances_15.txt");
	...
	file_LS.open ("result/W_LS_15.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	file_Likeli.open ("result/W_Likeli_15.txt");
	for(int i = 0; i < W_init.rows(); ++i){
		file_LS << W_init.row(i) << "\n";
	}
	file_Likeli.close();
	...
	file_final.open ("result/W_final_15.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	info_var_file.open ("result/info_var_15.txt");
	...
	boot_var_file.open ("result/boot_var_15.txt");
