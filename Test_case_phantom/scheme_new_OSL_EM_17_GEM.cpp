...
g++ scheme_new_OSL_EM_17_GEM.cpp -o test_17_2D -I /usr/include/eigen3 -O3 -lgsl -lgslcblas -lm



nohup ./test_17_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0 > test_17_2D.out & 
./test_17_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0
...
double Q_OSL_per_voxel(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	for(int j = 0; j < m; ++j) {
		...
		//if(i == 48864){
		//	Debug0("i: " << i << " , W.row(i): " << W.row(i) << "\t W_old.row(i)" << W_old.row(i) <<  
		//			"\tr.row(i) " << r.row(i) << 
					// "\nr(i,j) v_old_i(j) SQ(sigma(j)): " << r(i,j) << " " << v_old_i(j) << " " << SQ(sigma(j)) << 
					// ";\n tmp2: " << tmp2 << 
					// "; tmp3: " << tmp3 << " added: " << v_i(j)*(- 0.5*v_i(j) + r(i,j)*tmp3)/SQ(sigma(j)) << 
		//			" likeli_sum: " << likeli_sum 
					// << 
					//"\n v_i: " << v_i.transpose() << "\t v_old_i: " << v_old_i.transpose() << "\n"
		//			);
		//}
	}
	...
	if(i == 48864){
		Debug0("W.row(i): " << W.row(i) << "; likeli_sum: " << likeli_sum);
	}
...
Vector_eig Q_OSL_grad_per_voxel(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	for(int k = 0; k < 3; ++k){
		...
		for(int j = 0; j < m ; ++j){
			...
			if(i == 48864){
				...
				//Debug0("grad:::: tmp2: " << r(i,j)/SQ(sigma(j)) << ", tmp3: "<< tmp3);
			}
	...
	if(i == 48864){
		//Debug0("W_grad: " << W_grad.transpose());
	}
...
// I guess, it's the declaration
  public:
	...
	T value(const TVector &x) {
		...
		beta(0) = x(6); beta(1) = beta_y; beta(2) = beta_z;
...
// Comment this Gradient part if you don't want to feed the gradient:
	...
	// Suspect problem in beta - Subrata - run without grad.
...
// Change abs_diff, maxiter etc
	...
	//if(penalized){
		...
		MRF_optim<double> f_2(Eigen::VectorXd::Ones(7), MRF_obj);
		...
		Eigen::VectorXd x_MRF(7), lb_MRF(7), ub_MRF(7), x_MRF_old(7);
		...
		ub_MRF.segment(0, 6+1) =  255*Vector_eig::Ones(6+1);
		lb_MRF.segment(0, 6+1) = -255*Vector_eig::Ones(6+1);
		lb_MRF(0) = 1e-5; lb_MRF(0+3) = 1e-5; lb_MRF(0+5) = 1e-5;	// Diagonals of Cholesky to be BDD away from 0
		lb_MRF(0+6) = 1e-5;											// beta forecefully made BDD away from 0
		//ub.segment(0, 6+1) *= 15;		// Get's sqaured
		//lb.segment(0, 6+1) = 0.00001*Vector_eig::Ones(6);
	...
	while(iter < maxiter){
		...
		if(penalized){
			...
			Debug1 ("x_MRF at first: " << x_MRF.transpose());
			...
			beta(0) = x_MRF(6); beta(1) = 1.0; beta(2) = 0.0;
		...
		//for(int i = 73; i < 75; ++i) {
			...
			*/
			...
			// Test 
			if(i == 48864){
				Debug0 ("value of i: " << i << "\t x at first: " << x.transpose());
				Debug0 ("f(x) at first:" << old_val);
			}
			...
			// Test
			if(i == 48864){
				Debug0("Solver status: " << solver_2.status());
				Debug0("Final criteria values: " << "\n" << solver_2.criteria());
				
				Debug0 ("value of i: " << i << "\t x new: " << x.transpose());
				Debug0 ("f(x) new:" << fx << "\n");
			}
...
Matrix_eig Var_est_test_mat(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	assert(A.rows() == 3*n);
	save_sparse(A, "Hessian_Matrix_17_new.csv", 1);
...
int main(int argc, char * argv[]) {
	...
	file_LS.open ("result/W_LS_17_new.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	file_Likeli.open ("result/W_Likeli_17_new.txt");
	for(int i = 0; i < W_init.rows(); ++i){
		file_LS << W_init.row(i) << "\n";
	}
	file_Likeli.close();
	...
	file_final.open ("result/W_final_17_new.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	*/
	...
	info_var_file.open ("result/info_var_17_new.txt");
	...
	boot_var_file.open ("result/boot_var_17_new.txt");
