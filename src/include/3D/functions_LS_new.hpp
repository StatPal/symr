...
*/
double l_star_LS(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta,
...
*/
template<typename T>
class MRF_optim_nnnew_3D : public cppoptlib::BoundedProblem<T> {		// I guess it inherits
  public:
  ...
  public:	
	MRF_optim_nnnew_3D(const TMatrix_row &W1_, MRF_param &MRF_obj_optim_) : 
...
*/
template<typename T>
class Likeli_optim_nnnew_3D : public cppoptlib::BoundedProblem<T> {			// Likeli_optim_nnnew_3D is inheriting fn from cppoptlib::BoundedProblem
  public:
  ...
  public:
	Likeli_optim_nnnew_3D(MRF_param &MRF_obj_optim_, const TMatrix_row& r_, TMatrix_row& Theta_, 
	...
	T value(const TVector &x) {
		...
		//Rice part://
		for(int j = 0; j < m; ++j) {
			likeli_sum += v_i(j)*(- 0.5*v_i(j) + r(i,j))/SQ(sigma(j));
		}
...
*/
void LS_new_optim_3D(Matrix_eig_row &W_init, Matrix3d_eig &Psi_inv, Vector_eig &beta, 
...
// Change
	...
	old_likeli = l_star_LS(W_init, Psi_inv, beta, TE_example, TR_example,
...
//		Debug0("Number of possible background voxels: " << (black_list.sum()));
	...
	//if(penalized){
		...
		MRF_optim_nnnew_3D<double> f_2(W_init, MRF_obj);
		cppoptlib::LbfgsbSolver<MRF_optim_nnnew_3D<double>> solver_2;
	...
	while(iter < maxiter){
		...
		// * Loop over voxels: * //
		#pragma omp parallel default(none) firstprivate(x, old_val, fx, v_old_i) shared(MRF_obj, Theta, W_init, W_old, W_old_reserve, tmp2,    r, penalized, lb, ub, sigma, TE_example, TR_example, Psi_inv, beta,   n, m, verbose, verbose2, bad_count_o, bad_count_o_2, nan_count, Rcpp::Rcout, checkerboard_white, black_list)		// Check v_old_i, tmp2 -- CAREFULLY - Subrata
		{
		
			// lb, ub, etc would be shared
			// beta, Psi_inv would be Private???? -- no, they are not changed -- shared
			// 
			Likeli_optim_nnnew_3D<double> f(MRF_obj, r, Theta, W_init, W_old, penalized, lb, ub,
			...
			cppoptlib::LbfgsbSolver<Likeli_optim_nnnew_3D<double>> solver;			// For MRF parameters!
		...
		current_best_likeli = l_star_LS(W_init, Psi_inv, beta, TE_example, TR_example,
									 sigma, r, MRF_obj, penalized);
