...
*/
double l_star_LS_2D(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta,
...
*/
void LS_new_optim(Matrix_eig_row &W_init, Matrix3d_eig &Psi_inv, Vector_eig &beta, 
...
// Change
	...
	old_likeli = l_star_LS_2D(W_init, Psi_inv, beta, TE_example, TR_example,
...
//		Debug0("Number of possible background voxels: " << (black_list.sum()));
	...
	while(iter < maxiter){
		...
		current_best_likeli = l_star_LS_2D(W_init, Psi_inv, beta, TE_example, TR_example,
									 sigma, r, MRF_obj, penalized);
