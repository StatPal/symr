...
virtualenv_install("r-reticulate", c("scikit-image", "nibabel"))
...
pred_LS[mask==1,] <- 0
for(i in 1:9){
    pred_3D = array(pred_LS[,i], shapes)
...
DEEP_LEARNING_COMPARE <- FALSE # Change this to TRUE if DL results are needed.

if(DEEP_LEARNING_COMPARE){
  
  library(reticulate)
  virtualenv_install("r-reticulate", c("torch", "torchvision", "torchaudio", "matplotlib", "pandas", "joblib"))
  
  
  if(!dir.exists("DeepSynMRI/LS-py")) dir.create("DeepSynMRI/LS-py")
  if(!dir.exists("DeepSynMRI/DL_smooth"))  dir.create("DeepSynMRI/DL_smooth")
  
  # Run DL and save values
