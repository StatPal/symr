...
g++ All_possible_25_GEM.cpp -o all_25_2D -I /usr/include/eigen3 -O3 -lgsl -lgslcblas -lm




./all_25_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0

./all_25_2D ../data/new_phantom.nii Dummy_sd.txt 0

./all_25_2D ../Read_Data/small_phantom.nii Dummy_sd.txt 0

nohup ./all_25_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0 > all_25_2D.out & 
...
// Change
	...
	while(iter < maxiter){
		...
		if(verbose)
			Debug1("abs diff. (W_old - W_new):" << abs_sum(to_vector(W_old) - to_vector(W_init)));
...
//			// Divide into train and test:
//			std::vector<int> train_ind = {};
//			std::vector<int> test_ind = {i};
//			
//			for(int j = 0; j < m_total; ++j){
//				if(j != i)
//					train_ind.push_back(j);
//			}
