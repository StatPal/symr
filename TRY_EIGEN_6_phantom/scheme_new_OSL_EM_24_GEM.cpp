...
* Same as 23 - but rewritten from 23 to check same or not


* To compile:

g++ scheme_new_OSL_EM_24_GEM.cpp -o test_24_2D -I /usr/include/eigen3 -O3 -lgsl -lgslcblas -lm




./test_24_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0

./test_24_2D ../Read_Data/small_phantom.nii Dummy_sd.txt 0

nohup ./test_24_2D ../Read_Data/new_phantom.nii Dummy_sd.txt 0 > test_24_2D.out & 
...
double Q_OSL_per_voxel(const Matrix_eig_row &W, const Matrix3d_eig &Psi_inv, const Vector_eig &beta, 
	...
	if(penalized){
		...
		//if(i == 99){
		//	Debug0("MRF_obj.MRF_log_likeli_num_i_new(W.row(i), W, Psi_inv, beta, i): " << 
		//		MRF_obj.MRF_log_likeli_num_i_new(W.row(i), W, Psi_inv, beta, i) << "\n");
		//}
		
		likeli_sum += MRF_obj.MRF_log_likeli_num_i_new(W.row(i), W, Psi_inv, beta, i);
	}
...
int main(int argc, char * argv[]) {
	...
	file_performance.open ("result/Performances_24.txt");
	...
	file_LS.open ("result/W_LS_24.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	file_Likeli.open ("result/W_Likeli_24.txt");
	for(int i = 0; i < W_init.rows(); ++i){
		file_LS << W_init.row(i) << "\n";
	}
	file_Likeli.close();
	...
	file_final.open ("result/W_final_24.txt");
	for(int i = 0; i < W_init.rows(); ++i){
