...
g++ scheme_new_OSL_EM_21_GEM.cpp -o test_21_2D -I /usr/include/eigen3 -O3 -lgsl -lgslcblas -lm
...
int main(int argc, char * argv[]) {
	...
	file_performance.open ("result/Performances_21.txt");
	...
	file_LS.open ("result/W_LS_21.txt");
	for(int i = 0; i < W_init.rows(); ++i){
	...
	file_Likeli.open ("result/W_Likeli_21.txt");
	for(int i = 0; i < W_init.rows(); ++i){
		file_LS << W_init.row(i) << "\n";
	}
	file_Likeli.close();
	...
	file_final.open ("result/W_final_21.txt");
	for(int i = 0; i < W_init.rows(); ++i){
