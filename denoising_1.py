...
for target in range(9):
	...
	for slice_no in range(181):
		...
		if scaling_factor > 0:
			...
			q = plot_image_grid([np.clip(out_np, 0, 1), img_np], factor=13, name="intermediate_figs/err_1_"+str(target)+"_last.pdf");
	...
	pd.DataFrame(dat_target.reshape(181* 217* 181)).to_csv("Final_err_1_"+str(target)+".csv", header=None, index=None)
	time.sleep(120)
